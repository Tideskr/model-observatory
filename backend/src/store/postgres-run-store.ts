...
export class PostgresRunStore implements RunStore {
  ...
  async transition(runId: string, status: RunStatus, eventType: string, payload: Record<string, unknown> = {}): Promise<RunRecord> {
    const client = await this.pool.connect()
    try {
      await client.query('BEGIN')
    ...
    try {
      ...
      const selected = await client.query<RunRow>(
        `SELECT ${runColumns} FROM private_runs
         WHERE (status = 'queued' OR (status IN ('provisioning','running','scoring') AND lease_expires_at < now()))
           AND expires_at > now()
         ORDER BY created_at
      ...
      const updated = await client.query<RunRow>(
        `UPDATE private_runs SET status='provisioning',worker_id=$2,
      ...
      await appendAudit(client, 'run.provisioning', current.id, { status: 'provisioning', worker_id_hash: workerId })
      await client.query('COMMIT')
      return mapRun(updated.rows[0]!)
    } catch (error) {
      await client.query('ROLLBACK')
  ...
  async renewLease(runId: string, workerId: string, leaseSeconds: number): Promise<boolean> {
    const result = await this.pool.query(
      `UPDATE private_runs SET lease_expires_at=now()+($3::text||' seconds')::interval
       WHERE id=$1 AND worker_id=$2 AND status IN ('provisioning','running','scoring')`,
  ...
  async finalize(
    ...
    try {
      ...
      const updated = await client.query<RunRow>(
        `UPDATE private_runs SET status=$2,finished_at=now(),lease_expires_at=NULL WHERE id=$1 RETURNING ${runColumns}`,
        [runId, status],
      )
