...
class AppState:
    def __init__(self, runs_root: Path):
        ...
        self.observatory = ObservatoryApi()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _observatory_origin_allowed(self) -> bool:
        origin = self.headers.get("Origin")
    ...
    def _send_json(self, value: Any, status: int = 200, *, cors: bool = False) -> None:
    ...
    def _send_problem(self, problem: ApiProblem) -> None:
        self._send_json({"code": problem.code, "detail": problem.detail}, problem.status, cors=True)
    ...
    def do_OPTIONS(self) -> None:
        ...
        if path != "/status" and not path.startswith("/api/v1/private-runs"):
            self.send_error(404)
            return
        if not self._observatory_origin_allowed():
            return
    ...
    def do_GET(self) -> None:
        path = urlsplit(self.path).path
        if path == "/status":
            if self._observatory_origin_allowed():
                self._send_json({
            ...
            try:
                ...
                if action == "events":
                    try:
                        ...
                        raise ApiProblem(400, "invalid_event_cursor", "Last-Event-ID must be an integer") from exc
                    events = self.server.state.observatory.events(
                        run_id,
                        self._owner_token(),
                        cursor,
                        self.server.state,
                    ...
                    report = self.server.state.observatory.report(
                        run_id,
                        self._owner_token(),
                        self.server.state,
                    )
                    self._send_json(report, cors=True)
    ...
    def do_POST(self) -> None:
        path = urlsplit(self.path).path
        run_match = re.fullmatch(r"/api/v1/private-runs/([0-9a-f-]+)/cancel", path)
        if path in {"/api/v1/private-runs/quote", "/api/v1/private-runs"} or run_match:
            if not self._observatory_origin_allowed():
                return
            try:
                body = self._body()
                if path == "/api/v1/private-runs/quote":
                    response = self.server.state.observatory.issue_quote(body)
                elif path == "/api/v1/private-runs":
                    response = self.server.state.observatory.create_run(
                        body,
                        self.headers.get("Idempotency-Key", ""),
                        lambda payload: self._start_detector(payload, send_response=False),
                    )
                else:
                    response = self.server.state.observatory.cancel(
                        run_match.group(1),
                        self._owner_token(),
                        self.server.state,
                        lambda: self._stop_detector(send_response=False),
                    )
                self._send_json(response, cors=True)
            except ApiProblem as problem:
                self._send_problem(problem)
            except (KeyError, TypeError, ValueError) as exc:
                self._send_problem(ApiProblem(400, "invalid_request", _safe_exception_message(exc)))
            except Exception as exc:
                self._send_problem(ApiProblem(500, "local_runner_error", _safe_exception_message(exc)))
            return
    ...
    def _start_detector(self, body: dict[str, Any], *, send_response: bool = True) -> dict[str, Any]:
        ...
        worker.start()
        result = {"started": True, "session_id": session_id, "official": config["official"], "config_hash": config["config_hash"]}
        if send_response:
            self._send_json(result)
        return result

    def _stop_detector(self, *, send_response: bool = True) -> dict[str, Any]:
        result = {
            "accepted": False,
            "stopping": False,
            "session_id": None,
